...
- Deprecated `%not_in%` in favor of the `base` R `%notin%` operator
  (available in `R 4.6.0`). Calls to `%not_in%` now produce a
  deprecation warning, and users are encouraged to migrate to the `base`
  operator.
