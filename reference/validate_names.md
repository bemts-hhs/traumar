...
This function checks if all column names of a data frame or tibble are
within a specified set of valid values. Depending on the specified type,
it will either throw an error, issue a warning, or send a message.
...
- input:
  ...
  A data.frame or tibble. validate_names() will run colnames(input) to
