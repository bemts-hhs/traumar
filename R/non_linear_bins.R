...
#' for each bin.
#'
#' @param data A `data.frame` or `tibble` containing the probability of survival data for a set of patients.
#' @param Ps_col The column in `data` containing the probability of survival values for a set of patients.
#' @param divisor1 A parameter to control the width of the probability of survival range bins.
#' Affects the creation of step sizes for the beginning of each bin range. Defaults to `5`.
#' @param divisor2 A parameter to control the width of the probability of survival range bins.
#' Affects the creation of step sizes for the beginning of each bin range. Defaults to `5`.
#' @param threshold_1 A parameter to decide where data indices will begin to create step sizes. Defaults to `0.9`.
#' @param threshold_2 A parameter to decide where data indices will end to create step sizes. Defaults to `0.99`.
#'
...
nonlinear_bins <- function(data,
  ...
  survival_data <- data |> dplyr::pull({{ col }}) |> sort()
