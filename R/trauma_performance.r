####################################################################################################
### A function to calculate trauma hospital performance based on robust and validated    ###########
### measures. M, W, and Z scores via (Flora, 1978; Boyd et al., 1987; Mock et al., 2004). ##########
####################################################################################################
...
trauma_performance <- function(df, Ps_col, outcome_col, outcome = 1, z_method = c("survival", "mortality"), diagnostics = FALSE) {
  ...
  MTOS_distribution <- dplyr::tibble(
    Ps_range = factor(c(
      "0.96 - 1.00",
      "0.91 - 0.95",
      "0.76 - 0.90",
